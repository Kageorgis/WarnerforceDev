  ...
  return {
    ...
    controller : function($scope) {
        ...
        $scope.onItemClick = function(item){
          ...
          }else{ // US#660 Multi Deal planning Account Owner Dropdown start
              angular.forEach($scope.options,function(option,index){
                if(item.value != option.value && (item.value === 'myAccount' || item.value === 'allAccount') ) {
                    option["selected"] = false;
                }else if(item.value != option.value && (item.value != 'myAccount' || item.value != 'allAccount')){
                    if(option.value == 'myAccount' || option.value == 'allAccount' ){
                        option["selected"] = false;
                    }
                }
            });
          }
        // US#660 Multi Deal planning Account Owner Dropdown end
...
.directive("crmLookup",function($timeout){
            return {
                ...
                scope : {
                  ...
                  dealRecordTypeName: "=",
                  placeHolder : "@",
				  dealSalesOrg: "="		//ER-007269 added to check and filter account for SDS and S2S
                },
                        ...
                        if($scope.searchStr && $scope.searchStr.trim().length > 1) {
                            ...
                            if($scope.objectName == 'Account'){
                                ...
                                if($scope.dealRecordTypeName && $scope.dealRecordTypeName.trim().length > 1){
									...
									//ER-007269 added to check and filter account for SDS and S2S-- Start--//
									else if(($scope.dealSalesOrg && $scope.dealSalesOrg.trim().length > 1) && 
											($scope.dealSalesOrg == '9500' || $scope.dealSalesOrg == '9403' ||
												$scope.dealSalesOrg == '2500' || $scope.dealSalesOrg == '2403') && 
											($scope.dealRecordTypeName == 'Physical_POS_Deal' || $scope.dealRecordTypeName == 'Physical_DFI_Deal' ||
												$scope.dealRecordTypeName == 'Physical_Planning_Deal')){
                                        SOQL_STR += " AND Video_Physical__c = true";
										SOQL_STR += " AND Games_Physical__c = false";
										SOQL_STR += " AND Music_Physical__c = false";
										SOQL_STR += " AND Video_Digital__c = false";
										SOQL_STR += " AND Games_Digital__c = false";
										SOQL_STR += " AND Music_Digital__c = false";
                                        SOQL_STR += " AND Planning_Customer__c = true";
										SOQL_STR += " AND Sales_Organization__c = '"+$scope.dealSalesOrg.trim()+"'";
									}
									//ER-007269 added to check and filter account for SDS and S2S -- End--//
                                }
                            }else{
